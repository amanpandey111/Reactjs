...
import { addLocalData,isUserExists,toggleSignUp,toggleSignIn } from '../../features/UserSlices'
import "./ui.css"

const SignUp = () => {
    // creating a state for the form
    const dispatch = useDispatch()
    const isUser = useSelector((state)=>state.app.userexists)
    ...
    useEffect(()=>{
        if(submitted){
            ...
            if(dd==null){
                ...
                dispatch(toggleSignUp(false))                              // If user sign up direct to login page 
                 dispatch(toggleSignIn("toggle"))                          // enabling login page
            }else if(dd.length==1){
                ...
                if(local[0].email==user.email){
                    ...
                    dispatch(isUserExists(true))                             // telling user exists and we cannot add data
                    // dispatch(toggleSignUp(false))
                    // dispatch(toggleSignIn(true))
                }else{
                    dispatch(isUserExists(false))                            // telling user does not exists and we can add data
                ...
                if(b){
                    ...
                    dispatch(isUserExists(false))                             // telling user doesn't exists and we can add data
                    local.push(user)
                    dispatch(addLocalData(local))                             // here we are calling addLocalData so we can store the entire data in our store
                    localStorage.setItem("userdata",JSON.stringify(local))
                    dispatch(toggleSignUp(false))
