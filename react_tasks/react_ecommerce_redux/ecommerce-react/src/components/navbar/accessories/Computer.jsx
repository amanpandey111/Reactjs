...
const Computer = ({isDirect}) => {
  //! here we will get the data of our computer
  let computer = useSelector((state)=>state.computer)
  ...
  function handleInputChange(e){
    ...
    console.log(value);
    if(checked){
      const update = computer.filter((curEle)=>{
  ...
  return (
    <section onClick={()=>setIsPopup(false)} className={isDirect?"computer-section":"computer-section-some"}  >
      <div className='common-bg'><h2>Computers</h2></div>
      {isDirect || <div className='computer-div-ul-some'>
        <ul>
          {
            computer.map((curEle)=>{
              if(curEle.id%2==0){
                return(
                  <li key={curEle.id}>
                    <NavLink to={`/computer/${curEle.id}`}><img src={curEle.image} alt="" /></NavLink>
                    <p>{curEle.company}</p>
      ...
      {isDirect && <div className='computer-div-ul'>
