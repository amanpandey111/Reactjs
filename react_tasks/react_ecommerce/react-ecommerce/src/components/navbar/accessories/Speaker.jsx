...
const Speaker = ({isDirect}) => {
  const {speaker,setIsPopup} = useContext(DataProvider)
  ...
  function handleInputChange(e){
    ...
    console.log(value);
    if(checked){
      const update = speaker.filter((curEle)=>{
  ...
  let company_name = speaker.map((curEle)=>curEle.company)
  console.log(company_name);
  let company_list = [...new Set(company_name)]
  console.log(company_list);

  return (
    <section onClick={()=>setIsPopup(false)} className={isDirect?"speaker-section":"speaker-section-some"}>
      <div className='common-bg'><h2>Speaker</h2></div>
      {isDirect || <div className='speaker-div-ul-some'>
        <ul>
          {
            speaker.map((curEle)=>{
              if(curEle.id%2==0){
                return(
                  <li key={curEle.id}>
                    <NavLink to={`/speakers/${curEle.id}`}><img src={curEle.image} alt="" /></NavLink>
                    <p>{curEle.company}</p>
      ...
      {isDirect && <div className='speaker-div-ul'>
