...
const Kitchen = ({isDirect}) => {
  const {kitchen,setIsPopup} = useContext(DataProvider)
  ...
  function handleInputChange(e){
    ...
    console.log(value);
    if(checked){
      const update = kitchen.filter((curEle)=>{
  ...
  let company_name = kitchen.map((curEle)=>curEle.company)
  console.log(company_name);
  let company_list = [...new Set(company_name)]
  console.log(company_list);

  return (
    <section onClick={()=>setIsPopup(false)} className={isDirect?"kitchen-section":"kitchen-section-some"} >
      <div className='common-bg'><h2>Kitchen</h2></div>
      {isDirect || <div className='kitchen-div-ul-some'>
        <ul>
          {
            kitchen.map((curEle)=>{
              if(curEle.id%2==0){
                return(
                  <li key={curEle.id}>
                    <NavLink to={`/kitchen/${curEle.id}`}><img src={curEle.image} alt="" /></NavLink>
                    <p>{curEle.company}</p>
            ...
            <form action="">
              {
                company_list.map((curEle)=>{
                  return(
                    <div key={curEle.id}>
                      <input type="checkbox" name='kitchen' value={curEle} onChange={handleInputChange} />
