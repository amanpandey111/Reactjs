...
const OneDetail = () => {
    ...
    const handleEnrolled = (data) => {
        ...
        console.log(usercourses);
        if(usercourses!==null){
            const transformedUsers = usercourses.map(user => ({                 //! here we are getting only email and their course id
            useremail: user.useremail,
            courses: user.usercourse.map(course => course.id)
            }));

            console.log(transformedUsers);
            // dispatch(enrollCourses(user[0].email,id,"add"))
            console.log(user[0].email,data.id);
            console.log(transformedUsers);
            if(transformedUsers.length==1){
                bool = transformedUsers[0].courses.some((id)=>id==data.id)
                console.log(bool);
            }
            else{
                let index = transformedUsers.findIndex((curEle)=>curEle.useremail==user[0].email)
                console.log(index);
                if(index>=0){
                    bool = transformedUsers[index].courses.some((id)=>id==data.id)
                    console.log(bool);
                }
            }
        }
