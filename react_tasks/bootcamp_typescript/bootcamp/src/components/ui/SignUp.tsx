...
const SignUp = () => {
    ...
    const isUser = useSelector((state:globalstore)=>state.userexists)
    ...
    const formValidation = (name?:string,value?:any,error?:errorobject) => {
        console.log(error);
        let error1 : {[key:string]:string | boolean} = {valid:true,...error}
    ...
    useEffect(()=>{
        if(submitted){
            ...
            let dd = JSON.parse(localStorage.getItem("userdata") ?? "null" )
            console.log(dd);
            if(dd==null){
                ...
                console.log(users);
                let user : any = users.at(-1)
                console.log(user);
                let local = JSON.parse(localStorage.getItem("userdata") ?? "null" )
                ...
                let user : any = users.at(-1)
                let local = JSON.parse(localStorage.getItem("userdata") ?? "null" )
                let b = local.some((curEle:any)=>curEle.email==user.email)
                console.log(b);
                if(b){
                    dispatch(isUserExists(true))                             //todo : telling user exists and we cannot add data
                }
                ...
                dispatch(addLocalData(local))                                // here we are calling addLocalData so we can store the entire data in our store
                localStorage.setItem("userdata",JSON.stringify(local))
            }
        }
    },[users])
  ...
  return (
    ...
    sx={{position:"absolute",top:"50%",right:"50%",transform:"translate(50%,40%)",bgcolor:"white",fontFamily:"'Times New Roman', Times, serif",width:"30rem",borderRadius:"2rem"}}
    >
        <Stack sx={{m:"1.3rem 2.8rem !important"}} className="ss-div1">
            <h3>Sign Up</h3>
            <p>Please Fill the Form To create an account</p>
        </Stack>
        <hr />
        <Box component="form" onSubmit={handleFormSubmit} className="ss-f1"
        sx={{m:"1.3rem 2.8rem !important"}}
        >
            <Stack direction="row" spacing="1.4rem !important" className="form-div1 flex-div">
                {/* <input
                 type="text"
            ...
            <Box sx={{mt:"0.8rem !important",height:"minimum"}} className="flex-div">
                {/* <input type="Email"
            ...
            <Stack sx={{justifyContent:"flex-start",alignItems:"flex-start",mt:"1rem !important"}} >
                <Button variant="contained" type="submit" sx={{width:"100%",py:"0.4rem !important"}} >Sign up</Button>
                <Typography sx={{mt:"0.5rem !important",fontFamily:"sans-serif"}} >Already have a account ? <Box component="span" sx={{color:"#0A5ED7",cursor:"pointer",textDecoration:"underline"}} onClick={(e)=>{
                    e.stopPropagation()
                    dispatch(toggleSignIn("toggle"))
                    dispatch(toggleSignUp(false))
