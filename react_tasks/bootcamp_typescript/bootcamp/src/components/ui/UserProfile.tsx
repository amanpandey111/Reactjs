...
const UserProfile = () => {
  ...
  const[checkbox,setCheckBox] = useState<{email:string,course:string[]}>({
    email:"",
    course:[]
  })

    const[updatewish,setUpdateWish] = useState<one_course[]>([])                         // I am not using It's value just to re-render the page i used
    // const dispatch = useDispatch()
    const maincourses = useSelector((state:globalstore)=>state.usercourses)            // see we are not using data but it causes the re-render for every time local storage change     
    const userdetail:userdetail1[] = useSelector((state:globalstore)=>state.userdetail)              // Getting user detail
    let usercourses = JSON.parse(localStorage.getItem("usercourses") ?? "null")      // getting complete courses for all users 
    console.log(usercourses);

    /* Let's Get the courses data for a particualr user */
    let userdata = JSON.parse(localStorage.getItem("userdata") ?? "null")            // Getting the userdetails with their personal detail as well courses & wishlist
    console.log(userdata);
    // console.log(userdetail[0].email);
    let my:userdetail1[]=[];
    if(userdata && userdetail[0]){
      my = userdata.filter((curEle:userdetail1)=>curEle.email==userdetail[0].email)
    }
    

    // Let me start writing the code for wishlist
    // let wishlist = JSON.parse(localStorage.getItem("wishlist") ?? "null" )
    ...
    const handleRemoveCourse = (data:one_course) => {
      console.log(data);
      let index = my[0].courses.findIndex((curEle:one_course)=>curEle.id==data.id)
      console.log(index);
    ...
    const handleChange = (e:any,data:one_course) => {
      console.log(data.id);
    ...
    const deleteCheckBox = () => {+
      ...
      console.log(new_set);
      my[0].courses = my[0].courses.filter((curEle:one_course)=>!new_set.has(curEle.id))
      console.log(my[0].courses);
      console.log(userdata);
      localStorage.setItem("userdata",JSON.stringify(userdata))
    ...
    const handleRemoveWish = (id:string) => {
      ...
      let index = my[0].wishlist.findIndex((curEle:one_course)=>curEle.id==id)
      console.log(index);
    ...
    // }, [pathname]);
  return (
    <Box sx={{bgcolor:"#F2F2F2",height:"100vh !important",overflow:"auto",pb:"2rem !important"}}>
      ...
      <Box 
        ...
        sx={{border:"1px solid #718092",width:"40%",m:"auto !important",mt:"2.3rem",gap:"2rem !important",justifyContent:"center !important",
          alignItems:"center !important",py:"0.7rem !important",borderRadius:"0.5rem",mb:"1rem !important"}}
        >
          <div>
          ...
          <div>
            <div>
              {userdetail[0] && <p><Typography variant="body1" sx={{display:"inline",fontWeight:"600"}} >Student Name</Typography> : {userdetail[0].firstname} {userdetail[0].lastname}</p>}
              {userdetail[0] && <p><Typography variant="body1" sx={{display:"inline",fontWeight:"600"}} >Student email</Typography> : {userdetail[0].email}</p>}
            </div>
            {/* <div></div> */}
          </div>
        </Stack>
        ...
        sx={{mt:"2rem"}}
        >
          <h2 style={{fontWeight:"600",textAlign:"center",marginTop:"100px !important"}}>Enrolled Courses</h2>
          {checkbox.course.length>=1 && <Box sx={{width:"90%",m:"auto !important",display:"flex",justifyContent:"flex-end !important"}} >
            <Button variant="contained" sx={{bgcolor:"red",mt:"0.7rem !important",p:"0.4rem 0.6rem !important"}}
          onClick={()=>{deleteCheckBox()}}
          className="disenroll-checked"
          >Disenroll checked courses</Button>
          </Box> }
          <Paper elevation={4} className="course_div"
          sx={{width:"90%",m:"auto !important",py:"1rem !important",borderRadius:"1rem",bgcolor:"#F2F2F2",mb:"1rem !important",mt:"1rem !important"}}
          >
            {my[0].courses.length>=1 && <Grid container spacing={2} style={{}} >
              {my[0].courses.map((curEle:one_course,index)=>{
                return <Grid size={{lg:12,md:12,sm:12,xs:12}} sx={{placeItems:"center"}} className="course_grid" >
                    <Stack direction="row" className="course-group" key={curEle.id}
                          sx={{width:"90%",alignItems:"center",justifyContent:"space-between !important",m:"auto",mb:"1rem"}}
                    ...
                    <Stack sx={{lineHeight:"1.3rem !important",width:"15rem !important",pt:"0.6rem !important",textAlign:'center !important'}} className="course_para" >
                      <p>{curEle.name}</p>
        ...
        >
          ...
          <h2 style={{fontWeight:"600",textAlign:"center",padding:"100px !important"}}>Whishlist Courses</h2> :
          ...
          sx={{width:"90%",m:"auto !important",textAlign:"center !important",mt:"1.3rem !important"}} 
          >
            {
              my && my[0]?.wishlist && my[0].wishlist.length>=1 &&  (
                my[0].wishlist.map((curEle:one_course)=>{
                  return <Grid size={{xl:3,lg:4,md:6,sm:12,xs:12}} key={curEle.id} className="wishlist-one-grid"
                           ...
                           <NavLink to={`/${curEle.id}`}>
                             <CardMedia
                               ...
                               component="img"
                               image={curEle.image}
                               sx={{borderRadius:"1rem",height:"10rem",width:"20rem",m:"auto !important",mt:"0.6rem !important",mb:"0.5rem !important"}}
                             />
                           </NavLink>
                           <Stack spacing={2} >
